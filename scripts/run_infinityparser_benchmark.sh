...
# Runs Infinity Parser benchmark, measuring both olmOCR-bench performance and per document processing performance
#   ./scripts/run_infinityparser_benchmark.sh
...
IMAGE_TAG="olmocr-benchmark-infinityparser-${VERSION}-${GIT_HASH}"
...
# Shell script to run Infinity Parser conversions for benchmark
run_infinityparser_shell = dedent("""\
bash -lc 'set -euo pipefail
PDF_ROOT="olmOCR-bench/bench_data/pdfs"
TARGET_ROOT="olmOCR-bench/bench_data/infinityparser"
rm -rf "$TARGET_ROOT"
mkdir -p "$TARGET_ROOT"

# Install required dependencies for Infinity Parser
...
find "$PDF_ROOT" -type f -name "*.pdf" | while IFS= read -r pdf_path; do
    ...
    # Target location: olmOCR-bench/bench_data/infinityparser/${section}/${pdf_name}_pg1_repeat1.md
...
experiment_spec = ExperimentSpec(
    description=f"Infinity Parser Benchmark Run - Branch: {git_branch}, Commit: {git_hash}",
    budget="ai2/oe-base",
...
# Run the performance test with Infinity Parser
time parser --model infly/Infinity-Parser-7B --input /root/olmOCR-mix-0225_benchmark_set/ --output /root/olmOCR-mix-0225_benchmark_set_infinityparser --batch_size 128 --tp 1
""")
...
perf_task_spec_args = {
    "name": "infinityparser-performance",
    "image": ImageSource(beaker=f"{beaker_user}/{image_tag}"),
    "command": [
        "bash", "-c",
        " && ".join(perf_commands)
...
perf_experiment_spec = ExperimentSpec(
    description=f"Infinity Parser Performance Test - Branch: {git_branch}, Commit: {git_hash}",
    budget="ai2/oe-base",
